...
\end{figure}
		...
		\DIFaddbegin \begin{equation}\DIFadd{ Accuracy = (Ex_{tn} / Ac_{tn}) * 100 }\end{equation}	

			\DIFaddend Where \DIFdelbegin \DIFdel{T$_{p}$  }\DIFdelend \DIFaddbegin \DIFadd{Ex$_{tp}$  }\DIFaddend is the number of true positive sets extracted\DIFdelbegin \DIFdel{and T$_{n}$  is the }\DIFdelend \DIFaddbegin \DIFadd{, Ac$_{tn}$  is the actual number of true positive sets, Ex$_{tn}$ is the number of true negatives extracted and Ac$_{tn}$ is the actual }\DIFaddend number of true negative sets.
...
%	\subsection{ODACH}
	\DIFaddbegin \subsubsection{\DIFadd{Future Work}}
		...
		\DIFadd{Extracted images are stored in black and white and are only a few kilobytes in size, this will be useful as we expect that a large number of templates would be required in order to accurately identify an object of interest. For example, in our motivating scenario, an animal could be any distance from the camera, its legs could be in different positions or its angle it faces towards the camera could be different, giving many possible images. This process could be optimised by only matching regions between extracted images, such as the head or body, but this would require further experimentation.
}\DIFaddend 
