...
The results of the real world mode show that Zigbee is still a viable method for our motivating scenario but, for mission critical WSNs, where real time transmissions are a requirement, then a method with a higher transmission rate would be needed. However, in the worst case, 800 seconds from capture to delivery of a processed observations is still very positive and would save countless man hours, as well as allowing users of the network to act on most of the received sensed data immediately, without having to first manually process it.
