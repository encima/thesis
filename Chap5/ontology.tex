...
Due to the low cost and increasing capabilities of sensing hardware, the deployment of large-scale heterogeneous wireless sensor networks (WSNs) is becoming more applicable to a wide range of domains \cite{Kahn1999}. Because of this, the amount of data that is recorded by these networks is increasing, with the focus moving away from the power of the hardware and more towards maximising the efficient transmission of the data that is being sensed. Attempts, such as COUGAR \cite{Yao2002}, to aggregate the data and platforms have been developed to store the data on the node, reacting to a user's request for data. Other approaches have been to allow the use of `natural language' to query sensors for data, using the processing power of the network to make inferences about what data has been sensed, an example of this is the Semantic Streams framework \cite{Whitehouse2006}.

This sensed data can be described as an occurrence (or an observation), where a sensing device has recorded an event in space and time, that has been caused by a trigger. For example. a motion sensor would record an occurrence of a movement trigger, at the location it is deployed.

We present an aligning ontology for representing the capture of occurrence data, within a sensor network, as well as the flow of knowledge for inferring the content of the sensed data.

The rest of this paper is structured as follows: Section 2 outlines our motivation for the development of the K-HAS ontology, Section 3 summarises existing ontologies and other background research, Section 4 explains the methodology we used for the development of the ontology, Section 5 details the results of using the methodology and Section 6 is our evaluation. Section 7 details our conclusions and highlights planned future work.

\section{Motivation}\label{motiv}
This ontology has been developed to support our work developing a wireless sensor network that is capable of utilising the knowledge of the environment it is deployed in, as well as the knowledge of the data it has sensed. This knowledge can be classified as local or global. We have defined local knowledge as knowledge of an area that has been gained through experience, or experimentation. The area we have been implementing this network has been running a network of wildlife cameras for many years and has pre-existing data that has been manually collected by humans, this ontology also aims to provide a fixed structure to store, and classify, the thousands of pre-existing wildlife images that we currently have.

A typical sensing node is capable of recording the data it has sensed and forwarding it to a neighbouring node, as well as routing sensed data from the neighbouring nodes. These nodes send all sensed data through the network, unaware of the content. This means that a large volume of data could be received at the endpoint which may contain false readings, empty readings and even duplicates from sensors that are close to each other.

We hypothesise that nodes will be able to use local knowledge to infer whether sensed data is interesting; this could be through the metadata of an image, the text within a temperature reading or linking the newly sensed data with data that has previously been classified. In turn, this increases the efficiency of the network by prioritising data that is inferred to be of a higher importance.

For example, a camera sensor node could be triggered to take a picture and also receive a picture from a neighbouring node. In a typical WSN, the node would forward the picture it had taken, as well as its own but, in a network that uses local knowledge, the node could see that the received picture has already been marked as important, forcing all nodes to prioritise this data before their own. This means that data received at the endpoint of the network is not chronological, or preferential to a particular node, rather it is based on the knowledge from data that has previously been sensed, as well as characteristics of the environment that have been encoded onto each node. 

To test this, we have created an Knowledge-based Hierarchical Architecture for Sensing (K-HAS), a tiered architecture that is comprised of three tiers that attempt to classify and filter sensed data before reaching the endpoint \cite{gwilliams2012local}. In this section, we provide a short overview of this architecture to support the reasons for developing the ontology.

\subsection{K-HAS}
Through a collaboration with Cardiff School of Biosciences, we have developed the K-HAS architecture and implemented variations of it in the Malaysian rainforest with a view to capture, process and store images of wildlife with a significant reduction for any human involvement. K-HAS uses local and global knowledge through the use of rules encoded onto sensing nodes and dynamically adapts to changes in the network, such as new nodes added or changes in the sensed data. A simple overview of the K-HAS architecture is shown in Figure \ref{khasarch} and explained in detail below.

\begin{figure}
\includegraphics{/home/encima/Development/latex/thesis/Chap5/figures/khasarch.png}
\caption{K-HAS Architecture}
\label{khasarch}
\end{figure}

\subsubsection{Data Collection Node}
Data Collection (DC) nodes can be standard sensing devices; low power with limited storage and knowledge-processing capabilities that allows them to operate in a \textit{forward-and-forget} fashion that means that they are unaware of their history of sensed data, as well as what is being sensed around them. At the time of deployment, these devices are encoded with a fixed knowledge base. This give the node some knowledge of its surroundings, such as location, bearing and projects it is involved with, and allows it to filter the data that it senses. For example, a camera node could be aware that it is facing a river that is closed off at night, so any images taken during that time can be discarded (or sent with the lowest priority).

While these nodes do not have the capabilities to classify sensed data, they can enrich it and simplify the classification process for the next tier of the network.

\subsubsection{Data Processing Node}
These nodes do not perform any direct sensing, but they serve a subset of DC nodes, holding a more dynamic knowledge base that encompasses of the DC nodes it serves. Data Processing (DP) nodes have increased knowledge-processing capabilities and their storage capacity is significantly larger; because of this, they are able to run a more comprehensive rule management system and store a history of sensed data from DC nodes.

When DP nodes receive data, rules are fired on every detail. For example, in the case of an image, rules are fired on the location of the DC node, the metadata of the file and the contents of the image; every aspect of the data is processed. If the data can be classified, through the rules, previous classifications or both, then it is sent on to the next tier. If the data cannot be classified, but some of the rules fired indicate that it may be of interest, then it is marked as such and passed on. Sometimes, no classification or inference about the data can be made, so it is sent on with no enrichment.

\subsubsection{Data Aggregation Node}
A Data Aggregation (DA) node serves as the endpoint for a K-HAS network, it holds a global knowledge base of every deployed node, as well as allowing users to administer the network. All the sensed data of the network is stored here and users are able to classify any data that has been received. If a user changes the classification from what was inferred by the DP node, then the DP node is informed of the change and performs any necessary updates to its knowledge base.

For example, if a DP node sends a collection of images that it believes is a deer, based on the facts that it is facing into the forest and the extracted object from the images matches an existing template, but a user classifies it as a boar (which the node was not originally aware of), then the node updates its knowledge base and stores the template of the boar for future classifications.

We found that we needed a structured format for the data being sent around the network, as well as a formal representation for the flow of knowledge. Developing an ontology allowed us to outline the core concepts that make K-HAS unique, as well as identify areas where we can reuse popular, existing tools and methodologies.
...
K-HAS was developed in order to provide a generic architecture for wireless sensor networks to utilise the local knowledge contained within their environment to process sensed data and, therefore, make more efficient use of the network bandwidth by prioritising sensed data that is deemed to be more valuable \cite{}. We have defined local knowledge as knowledge of an area that has been gained through experience, or experimentation.
...
OBOE focuses on the concept of an \textit{observation}, which is made up of an entity, a measurement and a characteristic \cite{Madin2007}. An example of an observation could be a researcher observing an animal (the entity) and recording the gender (the measurement) as male (the characteristic). A single observation could then consist of multiple observations within it, such as gender, location, species and the number of species observed. 
    ...
    \begin{figure}
    \centering
        \scalebox{0.63}{\includegraphics{/home/encima/Development/latex/thesis/Chap5/figures/OBOEcore.JPG}}
...
The key difference to note with O\&M is that \textit{observation} and \textit{measurement} are not just classes. They also denote an action.
...
\begin{lstlisting}[caption=An Observation of a Vehicle encoded in O\&M, label=omxml, breaklines=true, language=XML]
 ...
 The listing shows that an observation centres around a \textit{feature of interest} that can be a physical object and the measurement is the detection of the vehicle at the recorded location. Figure \ref{oandm} shows a basic diagram of the ontology. While the event of a feature is linked, it is clear that the main focus is on the observation and the measurement associated with it.
    ...
    \begin{figure}[h]
    \centering
	\includegraphics[scale=0.5]{/home/encima/Development/latex/thesis/Chap5/figures/o&m_ontology.png}
...
\lstinputlisting[language=XML, caption=Darwin Core Descriptor File, label=dwc:meta]{/home/encima/Development/latex/thesis/Chap5/figures/dwc_arch/meta.xml}
\end{minipage}

    \begin{figure}
    \centering
      \scalebox{0.53}{\includegraphics{/home/encima/Development/latex/thesis/Chap5/figures/dwca.png}}
...
\noindent\begin{minipage}{\textwidth}
\lstinputlisting[caption=Darwin Core Occurrence Data, label=dwc:set]{/home/encima/Development/latex/thesis/Chap5/figures/dwc_arch/set.csv}
\end{minipage}
...
In order to represent DwC occurrences in an ontological format, work has been done to represent Darwin Core terms, as an ontology, in OWL. Darwin-Semantic Web (Darwin-SW) \cite{dwc_sw} is the project that aims to do this and many of the core terms associated with an occurrence have already been formalised.
    ...
    \begin{figure}[h]
    \centering
        \scalebox{0.13}{\includegraphics{/home/encima/Development/latex/thesis/Chap5/figures/dsw.jpg}}
    \caption{Darwin-SW}
    \label{darwin-sw}
    \end{figure}
...
The Semantic Sensor Network (SSN) Ontology is the most fitting ontology for our requirements, as it covers systems processes and observations. Developed by the W3C after an extensive review of existing ontologies \cite{lefort2011semantic}, the SSN ontology is designed to allow focus on a variety of perspectives, such as the sensor within the network or the data that has been observed.
    ...
    \begin{figure*}
    \centering
        \scalebox{0.50}{\includegraphics{/home/encima/Development/latex/thesis/Chap5/figures/ssnont.jpg}}
...
For the evaluation, Uschold and King adopt the definition of \cite{gomez1995evaluation}: “to make a technical judgement of the ontologies, their associated software environment, and documentation with respect to a frame of reference ... The frame of reference may be requirements specifications, competency questions, and/or the real world”.

There are some well documented methods for evaluating ontologies in the literature, \cite{REF!} proposes using the competency questions, used in the first step, to ensure that they can all be fully answered by the finished ontology. 

\subsection{Documentation}
Although this step is listed at the end of the development cycle, it seems more fitting to document all major aspects of the ontology as it is being developed. Documentation of the ontology should include: any assumptions, all concepts introduced, all ontologies that have been incorporated (by whatever meeans) and any primitives used for the definitions of concepts.
...
Over the course of eighteen months, which involved 2 field visits and several brainstorming meetings, we identified the core concepts that the ontology would need to contain, as well as the properties that would link them. From this, it would seem that we followed the \textit{Top-Down} approach, but the first visit with domain experts also allowed us to identify some of the more specialist classes early on in the development cycle. Thus, it seems that the \textit{Middle-Out} suits our methodology more closely.
...
Mapping these concepts, a diagram of the base K-HAS ontology is shown in Figure \ref{khas_base_ont}. The next step is to create the ontology and map the concepts identified to existing ontologies within the same domain space(s).
    ...
    \begin{figure}[h]
    \centering
        \scalebox{0.13}{\includegraphics{/home/encima/Development/latex/thesis/Chap5/figures/khas_base.JPG}}
...
Once the concepts had been identified (and agreed upon), the ontology must be coded. We used Protege 4.2.2 \cite{noy2001creating} and implemented K-HAS in the Web Ontology Language (OWL), creating a core file that could be expanded; should we need to import existing ontologies.

\subsection{Build the Ontology - Integrate Existing Ontologies}
Our implementation of K-HAS is focussed on scientific observations. Because of this, the focal point of our existing ontology research is of ontologies that are centred around scientific observations, this allows us to create a more generic ontology that is still able to capture all of the semantic details associated with a wildlife observation.

As explained in Section \ref{bg}, our research of existing ontologies covered two categories: observation-centric and sensor-centric. We identified ontologies, within multiple domains, that satisfied many of K-HAS' requirements, but not all. Using the core concepts outlined in Section \ref{buildcapture}, we created a minimal ontology and used the results of our research to map K-HAS concepts to those that had already been identified.

During this process we found that we had determined concepts that mapped to existing ontologies, but may not share identical structures, or even the same name. For example, the OBOE ontology describes the concept of an occurrence, which is very similar to our occurrence and the occurrence in the Darwin Core ontology. When we found these mappings, we used \textit{sameAs} relationship to form a link that allows data stored according to these existing ontologies to map directly to K-HAS. However, the structure of an OBOE observation is, as outlined in Section \ref{OBOE}, is more generic for all types of scientific observation and depicts the observation of an entity, containing a measurement of a particular characteristic. Whereas the structure of a Darwin Core observation is more limited to scientific observations of taxa which allows it to have more predefined terms, such as location, species name and the evidence for the recorded individual. Because of this, it was difficult to create a structure that encapsulated both OBOE and Darwin Core.

Research showed that Darwin Core maps to K-HAS' requirements more completely, as well as the fact that there has been work to represent Darwin Core Observations in OBOE \cite{dwc_oboe}, K-HAS occurrences map directly to Darwin Core and elements that are similar to OBOE have been linked by the \textit{sameAs} relationship. This does mean that OBOE observations cannot map directly, but K-HAS, and Darwin Core, occurrences can be converted, if necessary.

For the sensor hardware of K-HAS, we found that the SensorML ontology maps directly to our concepts and we also realised that we did not need to recreate concepts that may already exist in popular ontologies outside of the domain spaces we researched. For example, the Friend of a Friend (FOAF) ontology \cite{brickley2010foaf} is an ontology designed to create machine-readable pages that describe people, so it is more logical that K-HAS reuses existing terms from popular ontologies to allow pre-existing data to be mapped with ease.

INCLUDE TABLE TO MATCH TERMS TO EXISTING ONTOLOGIES??

\subsection{Ontology}

Whilst researching popular ontologies, we became aware that some of classes identified for K-HAS were not satisfied by what is currently available. The final step for creating the ontology was to add these concepts to the linked ontologies, we call these \textit{extension concepts}, concepts unique to K-HAS which do not exist in any other ontology. Much of these terms were linked to the unique layered architecture of K-HAS and we defined them within a new K-HAS namespace, changing our ontology from an alingment of existing ontologies to an extension of these.
...
The final ontology is shown in Figure \ref{khas_ont} and shows how each concept maps to existing ontologies. The figure shows that there were only five extension concepts unique to K-HAS that can be subclassed from the \textit{person} concept in the FOAF ontology and \textit{node} in the SensorML ontology.
    ...
    \begin{figure}[h]
    \centering
        \scalebox{0.13}{\includegraphics{/home/encima/Development/latex/thesis/Chap5/figures/khas_ontology.JPG}}
...
Find all \textbf{Occurrences} of an \textbf{Individual} at a \textbf{Location} & Occurrence, Token, Individual, Location, Identification & Occurrence hasEvidence Token; Token isBasisForID Identification; Identification identifies Individual \\
Find all \textbf{Occurrence} within a specified \textbf{Date} and \textbf{Time} range & Occurrence; Event; Date; Time & Occurrence atEvent Event; Event hasTime Time; Event hasDate; Date \\
Find all \textbf{Data-Collection Nodes} that have recorded an \textbf{Occurrence} of an \textbf{Individual} & DCNode; Individual; Occurrence & Occurrence recordedBy DCNode; Occurrence occurrenceOf Individual \\  
Find all \textbf{Locations} of an \textbf{Individual} & Occurrence; Individual; Event; Location & Occurrence occurrenceOf Individual; Occurrence hasEvent Event; Event locatedAt Location \\
Find the storage location of a \textbf{Project} & Project, Data Aggregation Node & Project storedBy DataAggregationNode \\ 
Find all \textbf{Projects} containing an \textbf{Individual} & Individual, Project; Occurrence & Individual occurredIn Occurrence; Occurrence heldIn Project \\
Find all \textbf{Persons} involved in a \textbf{Project} & Person; Administrator; Worker; Project & Administrator hasProject Project; Worker hasProject Project \\
Find all the \textbf{Evidence} that supports an \textbf{Occurrence} & Occurrence; Token & Occurrence hasEvidence Evidence \\
Find all the \textbf{Types} of evidence that supports an \textbf{Occurrence} & Occurrence; Token; Type & Occurrence hasEvidence Token; Token isOfType Type \\
\hline 
\end{tabular}
\label{eval:comp_qs}
\end{table} 
...
We used the example files from the archive in Section \ref{bg:dwc} to extract the terms associated with an occurence and we mapped these terms to K-HAS concepts, the results can be seen in Table \ref{tab:dwc_eval}.
...
In this paper we have explained our need for an ontological representation of our WSN architecture, K-HAS, and presented an early version. We have described our methodology for development and shown how existing ontologies are not complete enough. The K-HAS ontology we present is an alignment of ontologies that are spread across multiple domains and provides a complete solution for a sensor network that deals with scientific observations and we believe this is the first alignment ontology that combines sensor-centric and observation-centric ontologies. We have also extended this alignment to include concepts that model the unique features of K-HAS, but this ontology should be suitable for any sensor network that deals with observations.
...
In the future we plan to fully implement a K-HAS network, with each node utilising the ontological structure to send knowledge and occurrences around the network. We would also like to modify the ontology so that it is less reliant on the Darwin Core structure because, while it is currently suitable for our requirements, this does limit the uptake of this ontology outside of the ecology domain. Ideally, work would be done to make the Darwin Core terms more modular and users will then be able to 'plug in' their occurrence structure of choice.
